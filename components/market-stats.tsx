...
import { formatCompactNumber, formatPercentage, marketStats, fearGreedData, FearGreedData } from "@/lib/mockData"
...
export function MarketStats() {
  const { data: fgData } = useAutoRefresh(fetchFearGreed, 600000)
  const currentFg = fgData ?? fearGreedData
  ...
  const fearGreedIndex = currentFg.value;
  let fgLabel = currentFg.classification;
      ...
      <div className="bg-card rounded-lg p-4 shadow border border-border">
        ...
        <div className="space-y-3">
          <div className="flex justify-between">
            ...
            <span className="font-medium">{marketStats.totalCryptos.toLocaleString('en-US')}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Active Exchanges</span>
            <span className="font-medium">{marketStats.totalExchanges.toLocaleString('en-US')}</span>
          </div>
        </div>
      </div>
